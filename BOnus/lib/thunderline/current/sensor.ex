defmodule Thunderline.Current.Sensor do
  @moduledoc """
  Sensor: computes v/s/ρ + affect, builds gate score g, runs PLL + Hilbert,
  leases in prewindow, commits on beat, logs NDJSON, and broadcasts HUD status
  including PLV, Rayleigh p-values, mean phases, and ON‑BEAT.
  """
  use GenServer
  alias Thunderline.Bus
  alias Thunderline.Somatic.Embed, as: E
  alias Thunderline.Somatic.Engine, as: Somatic
  alias Thunderline.Daisy
  alias Thunderline.Current.{PLL, Lease}
  alias Thunderline.Current.Hilbert, as: H
  alias Thunderline.Current.PLV, as: PLV
  alias Thunderline.Current.CircStats
  ...
  def init(_) do
    ...
    {:ok, %{pll: %PLL{}, hilb: H.new(63), prev: [], win: [], hist: [], h_prev: 0.0,
            commits_pll: [], commits_hil: []}}
  end

  # external API (resurrection + safe close)
  ...
  def handle_info({:token, tok}, s) do
    ...
    {commits_pll, commits_hil} =
      if PLL.gate?(pll, g) do
        case :ets.lookup(:daisy_lease, :lease) do
          [{:lease, lease}] ->
            unless Lease.expired?(lease) do
              :ets.delete(:daisy_lease, :lease)
              Thunderline.Cerebros.Inferencer.apply_injection(lease.inj && lease.inj[:shard] || lease.inj)
              ...
              Bus.broadcast_status(%{
                stage: "committed",
                phi_pll: pll.phi, phi_h: phi_h,
                plv_pll: Float.round(plv_pll, 3), plv_h: Float.round(plv_h, 3),
                p_pll: Float.round(p_pll, 4), p_h: Float.round(p_h, 4),
                rbar_pll: Float.round(rbar_pll, 3), rbar_h: Float.round(rbar_h, 3),
                mu_pll: mu_pll, mu_h: mu_h,
                on_beat: on_beat,
                phases_pll: Enum.take(commits_pll1, @spark_cap) |> Enum.reverse(),
              ...
              NDJSON.write(%{
                event: "commit",
                phi_pll: pll.phi, phi_h: phi_h,
                plv_pll: plv_pll, plv_h: plv_h,
                rbar_pll: rbar_pll, p_pll: p_pll,
                rbar_h: rbar_h, p_h: p_h,
                mu_pll: mu_pll, mu_h: mu_h,
                on_beat: on_beat
              })
    ...
    {:noreply, %{s | pll: pll, hilb: hilb, prev: win, win: win, hist: [win | Enum.take(s.hist, 128)],
                 h_prev: h_t, commits_pll: commits_pll, commits_hil: commits_hil}}
  end

  def handle_info({:event, _evt}, s), do: {:noreply, s}
  ...
  defp entropy(win) do
    ...
    chars |> Enum.frequencies() |> Map.values()
    |> Enum.reduce(0.0, fn c, acc -> p = c/total; acc - p * (:math.log(p + 1.0e-12)/:math.log(2)) end)
  end

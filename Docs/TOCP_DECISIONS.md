...
| DIP-TOCP-002 | Telemetry Taxonomy & Sampling | OPEN | Event list + HB sample 1/20; p95 latency, churn, reliability metrics dashboards |
...
- No process spawns with open sockets in scaffold phase.
 - Control frames MUST be signed when `security_sign_control=true`.
 - Replay window enforced: drops duplicates/stale > replay_skew_ms.

# Thunderline Codebase Status – September 2025
...
| Event governance | Event validator and taxonomy linter guardrails exist; Bus shim deprecation still pending (HC-02). | `Thunderline.Thunderflow.EventValidator`, `mix thunderline.events.lint`, `Thunderline.Bus` legacy shim references. |
...
- Outstanding instrumentation gaps:
  1. Bus shim usage telemetry (HC-02) needs implementation to track residual `Thunderline.Bus` invocations.
  2. Blackboard delegate usage counter (WARHORSE Week 1 delta) to confirm migration completion.
  3. DLQ dashboard surfacing remains TODO in `THUNDERLINE_DOMAIN_CATALOG.md`.

## 5. Risk & Action Register

- **HC-01**: Publish helper exists (`Thunderline.Thunderflow.EventBus.publish_event/1`), but telemetry span enrichment + linter gating remain before the item is fully green.
...
- **CI Hardening**: Extend GitHub Actions with release + dialyzer cache + `hex.audit` (HC-08).

## 6. Recommended Next Steps

1. Wire `mix thunderline.events.lint` into CI alongside telemetry span assertions for EventBus publish helper.
2. Codemod remaining `Thunderline.Bus` references to the canonical EventBus and emit deprecation telemetry.
3. Expand LiveView tests for the Cerebros dashboard to cover event-driven updates and empty states using `Phoenix.LiveViewTest` utilities.
...
5. Capture feature flag documentation for `features.ml_nas` runtime toggles in `FEATURE_FLAGS.md` (HC-10).

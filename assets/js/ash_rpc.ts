...
// ThunderlineThunderlinkResourcesTicket Schema
export type ThunderlineThunderlinkResourcesTicketResourceSchema = {
  __type: "Resource";
  __primitiveFields: "id" | "subject";
  id: UUID;
  subject: string;
};
...
export type ThunderlineThunderlinkResourcesTicketFilterInput = {
  and?: Array<ThunderlineThunderlinkResourcesTicketFilterInput>;
  or?: Array<ThunderlineThunderlinkResourcesTicketFilterInput>;
  not?: Array<ThunderlineThunderlinkResourcesTicketFilterInput>;
...
export type ListTicketsFields = UnifiedFieldSelection<ThunderlineThunderlinkResourcesTicketResourceSchema>[];

type InferListTicketsResult<
  Fields extends ListTicketsFields,
> = {
  results: Array<InferResult<ThunderlineThunderlinkResourcesTicketResourceSchema, Fields>>;
  hasMore: boolean;
  limit: number;
  offset: number;
  count?: number | null;
...
export type ListTicketsResult<Fields extends ListTicketsFields> = | { success: true; data: InferListTicketsResult<Fields> }
| {
    success: false;
    errors: Array<{
      type: string;
...
export async function listTickets<Fields extends ListTicketsFields>(
  config: {
  fields: Fields;
  filter?: ThunderlineThunderlinkResourcesTicketFilterInput;
  sort?: string;
  page?: (
    {
      limit?: number;
...
): Promise<ListTicketsResult<Fields>> {
  const payload = {
    action: "list_tickets",
  ...
  return result as ListTicketsResult<Fields>;
}


export type ValidateListTicketsResult =
  | { success: true }
  | {
      success: false;
      errors: Array<{
...
export async function validateListTickets(
  config: {
  headers?: Record<string, string>;
  fetchOptions?: RequestInit;
  customFetch?: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;
}
): Promise<ValidateListTicketsResult> {
  const payload = {
    action: "list_tickets"
  };
  ...
  const result = await response.json();
  return result as ValidateListTicketsResult;
}
...
export type CreateTicketValidationErrors = {
  subject?: string[];
};
...
export type CreateTicketFields = UnifiedFieldSelection<ThunderlineThunderlinkResourcesTicketResourceSchema>[];

type InferCreateTicketResult<
  Fields extends CreateTicketFields,
> = InferResult<ThunderlineThunderlinkResourcesTicketResourceSchema, Fields>;

export type CreateTicketResult<Fields extends CreateTicketFields> = | { success: true; data: InferCreateTicketResult<Fields> }
| {
    success: false;
    errors: Array<{
      type: string;
...
export async function createTicket<Fields extends CreateTicketFields>(
  config: {
...
): Promise<CreateTicketResult<Fields>> {
  const payload = {
  ...
  const result = await response.json();
  return result as CreateTicketResult<Fields>;
}


export type ValidateCreateTicketResult =
  | { success: true }
  | {
      success: false;
      errors: Array<{
...
export async function validateCreateTicket(
  config: {
  input: CreateTicketInput;
  headers?: Record<string, string>;
  fetchOptions?: RequestInit;
...
): Promise<ValidateCreateTicketResult> {
  const payload = {
    action: "create_ticket",
    input: config.input
  };
  ...
  const result = await response.json();
  return result as ValidateCreateTicketResult;
}

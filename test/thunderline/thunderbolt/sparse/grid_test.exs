defmodule Thunderline.Thunderbolt.Sparse.GridTest do
  ...
  describe "delete/2" do
    ...
    test "is idempotent" do
      ...
      grid = Grid.put(grid, {5, 5, 5}, value)
      grid = Grid.delete(grid, {5, 5, 5})
      grid = Grid.delete(grid, {5, 5, 5})  # Delete again
  ...
  describe "map_active/2" do
    test "transforms all active cells" do
      ...
      doubled = Grid.map_active(grid, fn _coord, v ->
        Enum.map(v, &(&1 * 2))
      end)
  ...
  describe "filter_active/2" do
    test "removes cells not matching predicate" do
      ...
      grid = grid
        |> Grid.put({1, 1, 1}, [1.0] ++ List.duplicate(0.0, 15))  # First channel = 1
        |> Grid.put({2, 2, 2}, [2.0] ++ List.duplicate(0.0, 15))  # First channel = 2
        |> Grid.put({3, 3, 3}, [3.0] ++ List.duplicate(0.0, 15))  # First channel = 3
      
      filtered = Grid.filter_active(grid, fn _coord, value ->
        hd(value) >= 2.0
      end)
      
      assert filtered.nnz == 2
      assert Grid.get(filtered, {1, 1, 1}) == filtered.default  # Filtered out
  ...
  describe "sparsity/1" do
    ...
    test "decreases as cells are added" do
      grid = Grid.new({10, 10, 10})  # 1000 cells total
      value = List.duplicate(1.0, 16)

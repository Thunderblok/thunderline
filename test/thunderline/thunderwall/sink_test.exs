defmodule Thunderline.Thunderwall.SinkTest do
  ...
  describe "GenServer operations" do
    ...
    test "archives PAC lineage", %{pid: _pid} do
      lineage = [
      ...
      {:ok, entry} = Sink.archive_pac_lineage("pac_test_1", lineage, %{})
    ...
    test "lists quarantine entries", %{pid: _pid} do
      Sink.quarantine_ca_run("run_list_1", :ca_failure, %{})
      Sink.quarantine_ca_run("run_list_2", :nan_detected, %{})

      entries = Sink.list_quarantine()

      assert length(entries) == 2
    end

    test "lists archive entries", %{pid: _pid} do
      Sink.archive_pac_lineage("pac_archive_1", [%{fitness: 0.1}], %{})
      Sink.archive_pac_lineage("pac_archive_2", [%{fitness: 0.2}], %{})

      entries = Sink.list_archive()

      assert length(entries) == 2
    end

    test "returns stats", %{pid: _pid} do
      Sink.quarantine_ca_run("run_stats_1", :ca_failure, %{})
      Sink.archive_pac_lineage("pac_stats_1", [], %{})
    ...
    test "runs GC", %{pid: _pid} do
      {:ok, collected} = Sink.run_gc()

      assert is_integer(collected)
      assert collected >= 0
    end

    test "restores from quarantine", %{pid: _pid} do
      {:ok, entry} = Sink.quarantine_ca_run("run_restore_1", :ca_failure, %{})

      {:ok, restored} = Sink.restore(entry.id)
    ...
    test "restore returns error for non-existent entry", %{pid: _pid} do
      result = Sink.restore("nonexistent_id")

      assert result == {:error, :not_found}
    end

    test "lists patterns after archiving", %{pid: _pid} do
      Sink.archive_pac_lineage("pac_pattern_1", [%{fitness: 0.1}], %{})

      patterns = Sink.list_patterns()
  ...
  describe "edge cases" do
    ...
    test "handles empty lineage", %{pid: _pid} do
      {:ok, entry} = Sink.archive_pac_lineage("pac_empty", [], %{})

      assert entry.metadata.lineage_depth == 0
    end

    test "handles quarantine with minimal metadata", %{pid: _pid} do
      {:ok, entry} = Sink.quarantine_trial("trial_min", :trial_timeout, %{})

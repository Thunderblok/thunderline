defmodule ThunderlineTest.TenancyPolicyTest do
  ...
  describe "Tenancy Policy Tests" do
    ...
    test "expert tenancy enforced", %{actor_a: actor_a, actor_b: actor_b} do
      {:ok, expert} =
        Expert.register(%{tenant_id: actor_a.tenant_id, name: "exp", version: "v1", metrics: %{}},
          actor: actor_a,
          tenant: actor_a.tenant_id
        )
    ...
    test "lineage edge tenancy enforced", %{actor_a: actor_a, actor_b: actor_b} do
      ...
      {:ok, edge} =
        Edge.connect(
          %{
            from_id: parent_fw.id,
            to_id: child_fw.id,
            edge_type: "derives",
            day_bucket: Date.utc_today()
          },
          actor: actor_a,
          tenant: actor_a.tenant_id
        )
      ...
      assert {:ok, []} =
               Edge.read(
                 actor: actor_b,
                 tenant: actor_b.tenant_id,
                 query: [filter: [id: edge.id]]
               )
    end

    test "export job isolated", %{actor_a: actor_a, actor_b: actor_b} do
      {:ok, job} =
        TrainingSlice.enqueue(%{tenant_id: actor_a.tenant_id, slice_spec: %{}},
          actor: actor_a,
          tenant: actor_a.tenant_id
        )

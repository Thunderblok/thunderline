defmodule Thunderline.RAG.ServingTest do
  ...
  describe "RAG.Serving" do
    test "starts with disabled state when feature flag is off" do
      ...
      {:ok, pid} = Thunderline.RAG.Serving.start_link([])
      assert Process.alive?(pid)

      # Verify it returns disabled error
      assert {:error, :rag_disabled} = Thunderline.RAG.Serving.embed("test")
      assert {:error, :rag_disabled} = Thunderline.RAG.Serving.generate("test", [])
    end

defmodule ThunderlineWeb.DashboardLive.Components.DomainPanel do
      ...
      <div class="space-y-3">
        <%= case @domain do %>
          <% :thundercore -> %>
            ...
            <.metric_row label="Success Rate" value={format_percentage(@metrics[:success_rate])} color="purple" />
          ...
          <% :thunderbit -> %>
            ...
            <.metric_row label="Efficiency" value={format_percentage(@metrics[:efficiency])} color="green" />
          ...
          <% :thunderblock -> %>
            ...
            <.metric_row label="Members" value={format_number(@metrics[:total_members])} color="green" />
            <.metric_row label="Channels" value={@metrics[:active_channels]} color="blue" />
            <.metric_row label="Messages" value={format_number(@metrics[:messages_today])} color="purple" />
          ...
          <% :thundergrid -> %>
            ...
            <.metric_row label="Zone Health" value={format_percentage(@metrics[:zone_health])} color="green" />
          
          <% :thundervault -> %>
            <.metric_row label="Records" value={format_number(@metrics[:records])} color="cyan" />
            <.metric_row label="Knowledge" value={format_number(@metrics[:knowledge_nodes])} color="green" />
            <.metric_row label="Memory" value={@metrics[:memory_usage]} color="blue" />
            <.metric_row label="Query Perf" value={format_percentage(@metrics[:query_performance])} color="purple" />
          
          <% :thundercom -> %>
            <.metric_row label="Fed Messages" value={format_number(@metrics[:federated_messages])} color="cyan" />
          ...
          <% :thundereye -> %>
            ...
            <.metric_row label="Error Rate" value={format_percentage(@metrics[:error_rate])} color="yellow" />
          ...
          <% :thunderchief -> %>
            ...
            <.metric_row label="Health" value={format_percentage(@metrics[:dashboard_health])} color="purple" />
          
          <% :thunderflow -> %>
            <.metric_row label="Events" value={format_number(@metrics[:events_processed])} color="cyan" />
          ...
          <% :thunderstone -> %>
            ...
            <.metric_row label="Compliance" value={format_percentage(@metrics[:compliance_rate])} color="blue" />
          ...
          <% :thunderlink -> %>
            ...
            <.metric_row label="Link Health" value={format_percentage(@metrics[:link_health])} color="blue" />
          ...
          <% :thundercrown -> %>
            ...
            <.metric_row label="Utilization" value={format_percentage(@metrics[:resource_utilization])} color="blue" />
  ...
  defp status_indicator(assigns) do
    ...
    <div class={[
      ...
      status_color(@status)
    ]} title={String.capitalize(to_string(@status))}></div>
    """
  end

defmodule ThunderlineWeb.ChatLive do
  ...
  def mount(_params, _session, socket) do
    ...
    # Only subscribe if authenticated (avoid nil.id crash)
    if socket.assigns.current_user do
      ThunderlineWeb.Endpoint.subscribe("chat:conversations:#{socket.assigns.current_user.id}")
    end
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    ...
    cond do
      ...
      socket.assigns[:conversation] ->
        ThunderlineWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
        ThunderlineWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")

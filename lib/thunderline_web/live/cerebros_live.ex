defmodule ThunderlineWeb.CerebrosLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:drift_stats, %{lambda: 0.0, corr_dim: 0.0, sample: 0, coherence: 0.0, updated_at: nil})
  ...
  def handle_event("start_haus", _params, socket) do
    Task.start(fn -> Cerebros.haus(search_profile: :conservative, epochs: 1, trial_timeout_ms: 5_000) end)
  ...
  def handle_event("demo_positronic", _params, socket) do
    case Cerebros.demo_positronic(min_levels: 2, max_levels: 3) do
      {:ok, m} -> {:noreply, assign(socket, positronic: %{params: m.param_count, summary: summarize_spec(m.spec)})}
      {:error, reason} -> {:noreply, put_flash(socket, :error, "Positronic demo failed: #{inspect(reason)}")}
    end
  end

  def handle_event("mock_drift_spike", _params, socket) do
    # Simulate a drift spike to test UI responsiveness
    send(self(), {:drift_update, %{lambda: :rand.uniform() * 0.2, corr_dim: 2.0 + :rand.uniform()*3.0, sample: socket.assigns.drift_stats.sample + 10}})
    {:noreply, socket}
  end
  ...
  def handle_info(:tick, socket) do
    ...
    age_ms = case stats.updated_at do
      nil -> 0
  ...
  def handle_info({:drift_update, payload}, socket) do
    ...
    stats = %{lambda: lambda, corr_dim: d2, sample: sample, coherence: recompute_coherence(lambda, d2), updated_at: DateTime.utc_now()}
    {:noreply, assign(socket, :drift_stats, stats)}
  end
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div class="flex space-x-2">
            <button phx-click="start_haus" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded">Run haus demo</button>
            <button phx-click="demo_positronic" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded">Positronic</button>
          </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <form phx-change="noop" phx-submit="run_matmul" class="space-y-2">
            ...
            <input name="size" type="number" min="128" step="64" value="512" class="w-full border rounded px-2 py-1" />
            <button class="px-3 py-1 bg-indigo-600 text-white rounded">Run Matmul</button>
          </form>
          <button phx-click="run_training" class="px-3 py-1 bg-green-600 text-white rounded">Training Benchmark</button>
          <%= if @matmul do %>
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div class={lyap_color(@drift_stats.lambda)}>{format_float(@drift_stats.lambda,4)}</div>
            </div>
            <div>
          ...
          <button phx-click="mock_drift_spike" class="px-3 py-1 bg-amber-600 text-white rounded text-xs">Mock Spike</button>
          <div class="text-xs text-gray-500">Real streaming metrics will replace this mock once Thundereye drift server is active.</div>
        </div>
      </div>
    </div>
    """

if System.get_env("ENABLE_NN_PLAYGROUND") == "true" do
  defmodule ThunderlineWeb.NNPlaygroundLive do
    ...
    def handle_event("update_neurons", %{"index" => idx, "value" => v}, socket) do
      ...
      layers =
        update_in(socket.assigns.layers, [Access.at(i), :neurons], fn _ -> clamp(n, 1, 256) end)
      ...
      <div class="px-6 py-4 space-y-6">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-4 text-xs text-gray-400">
            ...
            <select
              name="dataset"
              phx-change="select_dataset"
              class="bg-gray-800 text-white rounded px-2 py-1 border border-gray-600"
            >
              <%= for d <- @datasets do %>
                <option value={d} selected={d == @dataset}>{d}</option>
              <% end %>
            </select>
            <button phx-click="reseed" class="px-2 py-1 bg-indigo-600 text-white rounded">
              Reseed
            </button>
            <button phx-click="add_hidden_layer" class="px-2 py-1 bg-emerald-600 text-white rounded">
              Add Layer
            </button>
            <%= if @dirty? do %>
              ...
              <%= for {layer, idx} <- Enum.with_index(@layers) do %>
                <div class="p-3 rounded bg-gray-800 border border-gray-700 space-y-2">
                  ...
                  <div class="flex items-center gap-3 text-xs text-gray-400">
                    ...
                    <select
                      phx-change="set_activation"
                      phx-value-index={idx}
                      class="bg-gray-900 text-white rounded px-2 py-1 border border-gray-600"
                    >
              ...
              <%= for {act, idx} <- Enum.with_index(@activations) do %>
                <div>
                  <div class="text-xs mb-1 text-gray-400">
                    Layer {idx + 1} activations (min={Float.round(act.min, 4)} max={Float.round(
                      act.max,
                      4
                    )})
                  </div>
                  <div
                    class="grid gap-0.5"
                    style={"grid-template-columns: repeat(#{act.width}, minmax(0, 1fr));"}
                  >
    ...
    defp recompute_forward(socket) do
      ...
      {acts, _last} =
        Enum.reduce(layers, {[], dataset}, fn layer, {acc, input} ->
          w =
            Nx.random_uniform({input.shape[1], layer.neurons}, -0.5, 0.5,
              key: random_key(layer.neurons, socket.assigns.seed)
            )
    ...
    defp build_dataset(:gaussians, seed) do
      ...
      cluster = fn cx, cy ->
        Enum.map(1..32, fn _ -> [cx + :rand.normal() * 0.1, cy + :rand.normal() * 0.1] end)
      end
    ...
    defp shuffle(t, seed) do
      idx =
        Enum.shuffle(
          :rand.seed_s(:exsss, {seed, seed, seed}) |> elem(1),
          Enum.to_list(0..(t.shape[0] - 1))
        )

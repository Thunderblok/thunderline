    ...
    <div class="flex h-full w-full text-gray-200">
      ...
      <div class="w-64 bg-slate-900/80 backdrop-blur border-r border-slate-700/40 flex flex-col">
        <div class="p-4 border-b border-slate-700/40">
          <h1 class="text-lg font-semibold truncate" title={@community.community_name}>{@community.community_name}</h1>
          <p class="text-xs text-slate-400 mt-1">Channels: {@community.channel_count} · Members: {@community.member_count}</p>
        </div>
        <div class="flex-1 overflow-y-auto px-2 py-3 space-y-4">
          <%= for {category, channels} <- @channel_groups do %>
            <div>
              <div class="text-[10px] uppercase tracking-wide text-slate-400 px-1 mb-1">{category || "General"}</div>
              <div class="space-y-0.5">
                <%= for ch <- channels do %>
                  <.link navigate={~p"/c/#{@community.community_slug}/#{ch.channel_slug}"} class="flex items-center px-2 py-1.5 rounded hover:bg-slate-700/40 text-sm group">
                    <span class="text-slate-500 group-hover:text-slate-300 mr-2">#{channel_icon(ch.channel_type)}</span>
        ...
        <div class="p-2 border-t border-slate-700/40 flex gap-2">
          <button phx-click="toggle_ai" class="flex-1 text-xs bg-indigo-600/20 hover:bg-indigo-600/30 rounded px-2 py-1 border border-indigo-500/40">AI</button>
          <button phx-click="toggle_command" class="flex-1 text-xs bg-slate-600/20 hover:bg-slate-600/30 rounded px-2 py-1 border border-slate-500/40">⌘K</button>
        </div>
      </div>
      ...
      <div class="flex-1 flex items-center justify-center bg-slate-950/60">
        <div class="text-center max-w-md">
          ...
          <p class="text-sm text-slate-400">Choose a channel from the left to start chatting. Use AI panel for assistance.</p>
        </div>
      </div>
      ...
      <%= if @ai_panel_open do %>
        <div class="w-80 border-l border-slate-700/40 bg-slate-900/70 backdrop-blur p-4 flex flex-col">
          ...
          <p class="text-xs text-slate-400 mb-3">AshAI integration placeholder. Conversation context & tool usage will appear here.</p>
          <form phx-submit="noop" class="mt-auto flex flex-col gap-2">
            <textarea class="w-full h-24 text-xs bg-slate-800/60 rounded border border-slate-600/40 focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none p-2" placeholder="Ask the AI..." />
            <button class="text-xs bg-indigo-600/30 hover:bg-indigo-600/40 rounded px-2 py-1 border border-indigo-500/40 self-end">Send</button>
          </form>
        </div>
      <% end %>

      <%= if @command_mode do %>
        <div class="fixed inset-0 bg-black/50 flex items-start justify-center pt-32" phx-click="toggle_command">
          <div class="bg-slate-900/90 backdrop-blur rounded-lg w-[640px] border border-slate-700/50 shadow-xl" phx-click="noop">
            ...
            <div class="p-3">
              <input type="text" placeholder="Type a command..." class="w-full text-sm bg-slate-800/60 border border-slate-600/40 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500" />

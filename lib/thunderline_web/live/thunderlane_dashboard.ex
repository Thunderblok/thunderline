defmodule ThunderlineWeb.ThunderlaneDashboard do
  @moduledoc """
  Thunderlane Dashboard - Multi-scale coordination visualization

  Implements the immersive dashboard with:
  - Nested hexagonal lane patterns for multi-scale coordination
  - Radial burst consensus visualization
  - Flowing gradient performance streams
  - Multi-layer telemetry analysis

  Real-time data is provided by `ThunderlineWeb.Live.ApiClient` with
  fallback defaults to ensure the dashboard never crashes on data errors.

  PubSub: Subscribes to "thunderlane:dashboard" for real-time updates.
  """

  use ThunderlineWeb, :live_view
  use Phoenix.Component

  alias ThunderlineWeb.Live.ApiClient
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     assign(socket,
       page_title: "Thunderlane Dashboard",
       lane_configurations: [],
       consensus_runs: [],
       performance_metrics: [],
       telemetry_snapshots: [],
  ...
  def handle_info(:load_initial_data, socket) do
    ...
    try do
      lanes = ApiClient.fetch_lane_configurations()
      consensus = ApiClient.fetch_consensus_runs()
      performance = ApiClient.fetch_performance_metrics()
      telemetry = ApiClient.fetch_telemetry_snapshots()
      ...
      error ->
        Logger.error("[ThunderlaneDashboard] Failed to load data: #{inspect(error)}")

        {:noreply,
         assign(socket,
           loading: false,
           error: "Failed to load dashboard data: #{Exception.message(error)}"
         )}
    end
  end
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # Component: Hexagonal Lane Grid
  # ─────────────────────────────────────────────────────────────────────────────

  defp hexagonal_lane_grid(assigns) do
    ~H"""
    ...
    <style>
      ...
      .lane-node:hover { transform: rotate(-30deg) scale(1.3); }
      ...
      .hub-label { font-size: 0.6rem; font-weight: bold; color: #003D42; }
      ...
      .stat-label { font-size: 0.7rem; color: rgba(0, 255, 225, 0.7); }
    </style>
    """
  end

  # ─────────────────────────────────────────────────────────────────────────────
  # Component: Radial Consensus Burst
  # ─────────────────────────────────────────────────────────────────────────────

  defp radial_consensus_burst(assigns) do
    ~H"""
    <div class="consensus-burst-container">
      ...
      <div class="burst-visualization">
        ...
        <div
          ...
          class={"consensus-ray ray-#{rem(index, 8)}"}
          style={"--delay: #{index * 0.2}s"}
        >
          <div class={"ray-segment segment-1 #{consensus_intensity_class(run)}"}></div>
        </div>
      </div>
    ...
    <style>
      ...
      .center-core {
        ...
        border-radius: 50%;
        background: radial-gradient(circle, #FF6B35 0%, #F7931E 30%, #FFD23F 60%, #FFF1C1 100%);
      ...
      .ray-segment.high-intensity { background: linear-gradient(to right, #FF6B35, #FFD23F); }
      .ray-segment.medium-intensity { background: linear-gradient(to right, #8B5CF6, #6366F1); }

      .consensus-metrics { padding: 1rem 0; }

      .metric-bar { display: flex; align-items: center; gap: 1rem; }
      .metric-label { min-width: 80px; font-size: 0.8rem; color: rgba(255, 107, 53, 0.9); }
      .metric-progress {
        flex: 1;
        height: 8px;
      ...
      .metric-value { min-width: 40px; text-align: right; font-weight: bold; color: #FFD23F; }
    </style>
    """
  end

  # ─────────────────────────────────────────────────────────────────────────────
  # Component: Performance Gradient Flow
  # ─────────────────────────────────────────────────────────────────────────────

  defp performance_gradient_flow(assigns) do
    ~H"""
    <div class="performance-flow-container">
      ...
      <div class="flow-visualization">
        <div
          ...
          class={"performance-stream #{scale}-scale"}
        >
    ...
    <style>
      ...
      .performance-rings { position: relative; width: 100%; height: 100%; }
      ...
      .ring-1 { width: 60px; height: 60px; border-color: rgba(139, 92, 246, 0.6); }
      .ring-2 { width: 80px; height: 80px; border-color: rgba(0, 255, 225, 0.5); animation-delay: 0.5s; }
      .ring-3 { width: 100px; height: 100px; border-color: rgba(255, 107, 53, 0.4); animation-delay: 1s; }
      ...
      .scale-name { font-weight: bold; color: rgba(255, 255, 255, 0.9); }
      .scale-value { color: rgba(255, 210, 63, 0.8); }
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # Component: Telemetry Nested Layers
  # ─────────────────────────────────────────────────────────────────────────────

  defp telemetry_nested_layers(assigns) do
    ~H"""
    ...
    <style>
      ...
      .layer-0 { width: 280px; height: 280px; border: 3px solid rgba(255, 0, 128, 0.6); }
      .layer-1 { width: 220px; height: 220px; border: 3px solid rgba(255, 64, 129, 0.7); }
      .layer-2 { width: 160px; height: 160px; border: 3px solid rgba(255, 128, 171, 0.8); }
      .layer-3 { width: 100px; height: 100px; border: 3px solid rgba(255, 179, 209, 0.9); }
      ...
      .core-stats .stat-value { font-weight: bold; color: #FF0080; }
      .core-stats .stat-unit { color: rgba(255, 128, 171, 0.8); }
      ...
      .stream-dot.window { background: radial-gradient(circle, #00FFE1, #007B8A); }
      .stream-dot.burst { background: radial-gradient(circle, #FF6B35, #F7931E); }
      .stream-dot.anomaly { background: radial-gradient(circle, #FF0080, #FF4081); }
      .stream-dot.baseline { background: radial-gradient(circle, #8B5CF6, #6366F1); }
  ...
  defp update_lane_in_list(lanes, new_lane_data) do
    Enum.map(lanes, fn lane ->
      if lane.id == new_lane_data.id, do: Map.merge(lane, new_lane_data), else: lane
    end)
  end

  defp update_consensus_in_list(runs, new_consensus_data) do
    Enum.map(runs, fn run ->
      if run.id == new_consensus_data.id, do: Map.merge(run, new_consensus_data), else: run
    end)
  end

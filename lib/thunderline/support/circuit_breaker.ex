defmodule Thunderline.Support.CircuitBreaker do
  ...
  def call(service_key, fun) when is_function(fun, 0) do
    case get_state(service_key) do
      {:open, until_ms} when until_ms > System.monotonic_time(:millisecond) ->
  ...
  def get_circuit_state(service_key) do
    case get_state(service_key) do
      {:closed, _} -> :closed
      {:open, until_ms} when until_ms > System.monotonic_time(:millisecond) -> :open
      {:open, _} -> :half_open  # Expired cooldown

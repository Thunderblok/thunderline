defmodule Thunderline.Thunderflow.EventBuffer do
  @moduledoc """
  Rolling in-memory event buffer (ETS) for dashboard streaming.

  Domain: Thunderflow (event streams & observability)

  Responsibilities:
  - Fixed-size ring (configurable :limit, default 500)
  - Normalizes disparate PubSub/domain/telemetry events into a common map
  - Broadcasts normalized events on PubSub topic `dashboard:events`
  - Provides cheap snapshot retrieval for initial LiveView mount
  """
  use GenServer
  require Logger
  ...
  def init(opts) do
    ...
    state = %{limit: limit}
    Logger.info("[EventBuffer] started (limit=#{limit}) domain=thunderflow")
    {:ok, state}
  end

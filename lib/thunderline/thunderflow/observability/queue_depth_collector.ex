defmodule Thunderline.Thunderflow.Observability.QueueDepthCollector do
  ...
  @collection_interval Application.compile_env(:thunderline, [:queue_metrics, :collection_interval], 5_000)
  @trend_window_size Application.compile_env(:thunderline, [:queue_metrics, :trend_window_size], 60)
  @saturation_threshold Application.compile_env(:thunderline, [:queue_metrics, :saturation_threshold], 0.8)
  ...
  defp collect_queue_depths(state) do
    ...
    deltas = calculate_depth_deltas(current_depths, state.depth_history, timestamp - state.last_collection)
  ...
  defp analyze_queue_trends(samples) do
    ...
    Map.new(queue_names, fn queue ->
      queue_depths = Enum.map(samples, fn sample -> 
        Map.get(sample.depths, queue, 0) 
      end)
      
      queue_deltas = Enum.map(samples, fn sample ->
  ...
  defp check_queue_saturation(state) do
    ...
    saturated_queues = 
      Enum.filter(current_stats.current_depths, fn {queue, depth} ->
        max_size = Map.get(state.queue_configs, queue, 100)  # Default max

defmodule Thunderline.Thunderflow.Probing.Attractor do
  ...
  defp reliability_note(true, _p, _r, _min), do: "heuristics only; no scaling region validation performed"
  defp reliability_note(false, p, r, min), do: "insufficient data (points=#{p}, delay_rows=#{r}, min_points=#{min})"
  ...
  defp lyapunov_rosenstein(delay_vectors, opts) do
    ...
    if max_h < 2, do: %{lyap: 0.0, r2: 0.0, window: {0,0}}, else: rosenstein_fit(delay_vectors, max_h)
  end

  defp rosenstein_fit(vs, max_h) do
    # Build average divergence series
    series = for h <- 1..max_h do
      logs =
    ...
    {best, fit} = Enum.reduce(windows, {{0,0}, %{r2: -1.0}}, fn {s,e}=win, {_, best_fit} ->
  ...
  defp linreg(xs, ys) do
    ...
    mean_x = Enum.sum(xs)/n
    mean_y = Enum.sum(ys)/n
    {num, den} = Enum.zip(xs, ys) |> Enum.reduce({0.0,0.0}, fn {x,y},{a,b} -> {a + (x-mean_x)*(y-mean_y), b + :math.pow(x-mean_x,2)} end)
    slope = if den == 0.0, do: 0.0, else: num/den
    intercept = mean_y - slope * mean_x
    ss_tot = Enum.reduce(ys, 0.0, fn y, acc -> acc + :math.pow(y-mean_y,2) end)
    ss_res = Enum.zip(xs, ys) |> Enum.reduce(0.0, fn {x,y}, acc -> acc + :math.pow(y - (slope * x + intercept),2) end)

defmodule Thunderline.Thunderflow.Probing.Workers.ProbeAttractorSummaryWorker do
  ...
  def perform(%Oban.Job{args: %{"run_id" => run_id} = args}) do
    with {:ok, run} <- Ash.get(ProbeRun, run_id) do
      case Ash.read_one(ProbeAttractorSummary, [run_id: run.id]) do
        ...
        {:error, err} ->
          Logger.error("[ProbeAttractorSummaryWorker] read error run=#{run.id} err=#{inspect(err)}")
      end
    else
      {:error, err} -> Logger.error("[ProbeAttractorSummaryWorker] run not found #{run_id} err=#{inspect(err)}")
    end
    :ok
  end
  ...
  {:ok, _rec} = Ash.create(ProbeAttractorSummary, summary_map)
    :telemetry.execute([:thunderline, :probe, :attractor_summary], %{
    ...
    Logger.info("[ProbeAttractorSummaryWorker] created summary run=#{run.id} cd=#{summary_map.corr_dim} ly=#{summary_map.lyap} ro_r2=#{summary_map.lyap_r2} canonical=#{summary_map.lyap_canonical}")
  rescue
    e -> Logger.error("[ProbeAttractorSummaryWorker] failure run=#{run.id} error=#{Exception.message(e)}")
  end
  ...
  defp canonical_lyap(%{lyap_r2: r2, lyap: simple, lyap_window: _w} = s) when is_number(r2) do
    threshold = 0.6
    ro = Map.get(s, :lyap_rosenstein) || Map.get(s, :lyap) # fallback

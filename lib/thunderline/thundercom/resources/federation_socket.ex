defmodule Thunderline.Thundercom.Resources.FederationSocket do
  @moduledoc """
  ...
  attributes do
    ...
    attribute :bandwidth_limits, :map do
      ...
      default %{
        ...
        "max_bytes_per_minute" => 1048576, # 1MB
        "burst_allowance" => 200,
  ...
  belongs_to :target_community, Thunderline.Thundercom.Resources.Community do
  ...
  actions do
    ...
    create :create do
      ...
      accept [:socket_name, :socket_slug, :socket_type, :target_community_id, :target_realm_address,
              :federation_protocol, :trust_level, :connection_config, :message_routing,
              :sharing_policies, :security_config, :bandwidth_limits, :relay_config,
              :authentication_data, :tags, :metadata, :community_id]
    ...
    update :update do
      ...
      accept [:socket_name, :socket_type, :federation_protocol, :trust_level, :connection_config,
              :message_routing, :sharing_policies, :security_config, :bandwidth_limits,
              :relay_config, :tags, :metadata]
    end
    ...
    update :send_message do
      ...
      change fn changeset, context ->
        ...
        updated_queue = [queued_message | current_queue] |> Enum.take(1000) # Limit queue size
        updated_metrics = Map.put(current_metrics, "messages_sent",
      ...
      change after_action(fn _changeset, socket, context ->
        ...
        process_outbound_message(socket, context.arguments.message_data, context.arguments.priority)
    ...
    update :receive_message do
      ...
      change fn changeset, context ->
        ...
        updated_metrics = Map.put(current_metrics, "messages_received",
      ...
      change after_action(fn _changeset, socket, context ->
        ...
        process_inbound_message(socket, context.arguments.message_data, context.arguments.source_realm)
    ...
    update :heartbeat do
      ...
      change fn changeset, _context ->
        current_metrics = Ash.Changeset.get_attribute(changeset, :federation_metrics) || %{}
        uptime = Map.get(current_metrics, "connection_uptime", 0) + 30 # 30 second heartbeat
    ...
    update :log_error do
      ...
      change fn changeset, context ->
        ...
        updated_log = [error_entry | current_log] |> Enum.take(100) # Keep last 100 errors
        updated_metrics = Map.put(current_metrics, "error_count",
          Map.get(current_metrics, "error_count", 0) + 1)
      ...
      change after_action(fn _changeset, socket, context ->
        ...
        handle_federation_error(socket, context.arguments.error_type, context.arguments.error_message)
        {:ok, socket}
      end)
    end
    ...
    update :quarantine_message do
      ...
      change fn changeset, context ->
        ...
        updated_queue = [quarantine_entry | current_queue] |> Enum.take(500) # Limit quarantine size
  ...
  postgres do
    ...
    custom_indexes do
      index [:socket_slug, :community_id], unique: true, name: "federation_sockets_slug_community_idx"
    ...
    check_constraints do
      check_constraint :valid_target, "target_community_id IS NOT NULL OR target_realm_address IS NOT NULL"
      check_constraint :valid_metrics, "jsonb_typeof(federation_metrics) = 'object'"
      check_constraint :valid_trust_level, "trust_level IN ('untrusted', 'basic', 'verified', 'trusted', 'sovereign')"
    end
  end

defmodule Thunderline.Thundercom.Resources.Community do
  @moduledoc """
  ...
  has_many :channels, Thunderline.Thundercom.Resources.Channel do
      destination_attribute :community_id
    end

  has_many :pac_homes, Thunderline.Thundercom.Resources.PACHome do
      destination_attribute :community_id
    end

  has_many :community_roles, Thunderline.Thundercom.Resources.Role do
      destination_attribute :community_id
    end

  has_one :federation_socket, Thunderline.Thundercom.Resources.FederationSocket do
      destination_attribute :community_id
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [:community_name, :community_slug, :community_type, :governance_model,
              :federation_config, :community_config, :resource_limits, :owner_id,
              :invitation_config, :community_policies, :tags, :metadata, :cluster_node_id]

      change fn changeset, _context ->
        changeset
        |> Ash.Changeset.change_attribute(:status, :initializing)
        |> Ash.Changeset.change_attribute(:member_ids, [Ash.Changeset.get_attribute(changeset, :owner_id)])
    ...
    update :update do
      ...
      accept [:community_name, :community_type, :governance_model, :federation_config,
              :community_config, :resource_limits, :invitation_config, :community_policies,
              :tags, :metadata]
    end
    ...
    action :activate, :struct do
      ...
      run fn input, _context ->
        ...
        Phoenix.PubSub.broadcast(
          Thunderline.PubSub,
          "thunderblock:communities",
          {:community_activated, %{community_id: community.id, community_slug: community.community_slug}}
        )
    ...
    action :add_member, :struct do
      ...
      run fn input, _context ->
        case Thunderblock.Domain |> Ash.get(Thunderblock.Resources.Community, input.arguments.id) do
          {:ok, community} ->
            {:ok, updated_community} = community
    ...
    update :_internal_add_member do
      ...
      change fn changeset, context ->
        ...
        updated_moderators = if role in [:moderator, :admin] and user_id not in current_moderators do
        ...
        case Thunderblock.Domain |> Ash.get(Thunderblock.Resources.Community, input.arguments.id) do
          {:ok, community} ->
            ...
            Phoenix.PubSub.broadcast(
              Thunderline.PubSub,
              "thunderblock:communities",
              {:community_suspended, %{community_id: updated_community.id, community_slug: updated_community.community_slug}}
            )
  ...
  # ===== PREPARATIONS =====
  preparations do
    prepare build(load: [:cluster_node, :zone_container, :channels, :pac_homes, :federation_socket])
  end

  # ===== PRIVATE FUNCTIONS =====
  defp provision_community_zone(community) do
    # Create dedicated execution zone for community
    {:ok, zone} = Thunderblock.Resources.ZoneContainer.create(%{

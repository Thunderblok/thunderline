defmodule Thunderline.Thunderbolt.Signal.Sensor do
  ...
  def boundary_close(timeout_ms \\ 200),
    do: GenServer.call(__MODULE__, {:boundary_close, timeout_ms}, timeout_ms + 100)

  def handle_cast({:restore, pll_map, echo}, s) do
    pll =
      case pll_map do
        %{"phi" => phi, "omega" => om, "eps" => eps, "kappa" => k} ->
          %PLL{phi: phi, omega: om, eps: eps, kappa: k}
    ...
    with {:ok, ev} <-
           Thunderline.Event.new(
             name: "system.status.resumed_prepare",
             source: :bolt,
             payload: %{stage: "resumed_prepare", phi_pll: pll.phi},
      ...
      {:error, reason} ->
        Logger.warning("event_publish_failed resumed_prepare #{inspect(reason)}")
  ...
  def handle_info({:token, tok}, s) do
    ...
    if PLL.prewindow?(pll) and :ets.lookup(:daisy_lease, :lease) == [] do
      ...
      with {:ok, ev} <-
             Thunderline.Event.new(
               name: "system.status.prewindow",
               source: :bolt,
               payload: %{stage: "prewindow", phi_pll: pll.phi, phi_h: phi_h, gate_score: g},
               meta: %{pipeline: :realtime},
               type: :system_status
             ),
           {:ok, _} <- EventBus.publish_event(ev) do
        ...
        {:error, reason} -> Logger.warning("event_publish_failed prewindow #{inspect(reason)}")
        _ -> :ok
      end
    end
        ...
        case :ets.lookup(:daisy_lease, :lease) do
          [{:lease, lease}] ->
            unless Lease.expired?(lease) do
              ...
              with {:ok, ev} <-
                     Thunderline.Event.new(
                       name: "system.status.committed",
                       source: :bolt,
                       payload: %{
                         stage: "committed",
                         phi_pll: pll.phi,
                         phi_h: phi_h,
                         plv_pll: Float.round(plv_pll, 3),
                         plv_h: Float.round(plv_h, 3),
                         p_pll: Float.round(p_pll, 4),
                         p_h: Float.round(p_h, 4),
                         rbar_pll: Float.round(rbar_pll, 3),
                         rbar_h: Float.round(rbar_h, 3),
                         mu_pll: mu_pll,
                         mu_h: mu_h,
                         on_beat: on_beat,
                ...
                {:error, reason} ->
                  Logger.warning("event_publish_failed committed #{inspect(reason)}")
    ...
    {:noreply,
     %{
       ...
       | pll: pll,
         ...
         prev: win,
         win: win,
         hist: [win | Enum.take(s.hist, 128)],
         h_prev: h_t,
         commits_pll: commits_pll,
         commits_hil: commits_hil
     }}
  end
  ...
  def handle_call({:boundary_close, _timeout_ms}, _from, s) do
    ...
    with {:ok, ev} <-
           Thunderline.Event.new(
             name: "system.status.paused",
             source: :bolt,
             payload: %{stage: "paused", reason: "boundary_close", phi_pll: s.pll.phi},
             meta: %{pipeline: :realtime},
             type: :system_status
           ),
         {:ok, _} <- EventBus.publish_event(ev) do
      ...
      {:error, reason} -> Logger.warning("event_publish_failed paused #{inspect(reason)}")
      _ -> :ok
    end

    {:reply,
     %{
       gate_ts: System.monotonic_time(:millisecond),
       phi_pll: s.pll.phi,
       echo_window: echo,
       pll_state: Map.from_struct(s.pll)
     }, s}
  end
  ...
  defp entropy(win) do
    ...
    |> Enum.reduce(0.0, fn c, acc ->
      p = c / total
      acc - p * (:math.log(p + 1.0e-12) / :math.log(2))
    end)
  end
  ...
  defp fallback_cosine(lhs, rhs) do
    ...
    dot =
      Enum.zip(aligned_lhs, aligned_rhs) |> Enum.reduce(0.0, fn {l, r}, acc -> acc + l * r end)

defmodule Thunderline.Thunderbolt.Signal.Sensor do
  ...
  with {:ok, ev} <- Thunderline.Event.new(name: "system.status.resumed_prepare", source: :bolt, payload: %{stage: "resumed_prepare", phi_pll: pll.phi}, meta: %{pipeline: :realtime}, type: :system_status),
       {:ok, _} <- EventBus.publish_event(ev) do :ok else
    {:error, reason} -> Logger.warning("event_publish_failed resumed_prepare #{inspect(reason)}")
    _ -> :ok
  end
  ...
  with {:ok, ev} <- Thunderline.Event.new(name: "system.status.paused", source: :bolt, payload: %{stage: "paused", reason: "boundary_close", phi_pll: s.pll.phi}, meta: %{pipeline: :realtime}, type: :system_status),
       {:ok, _} <- EventBus.publish_event(ev) do :ok else

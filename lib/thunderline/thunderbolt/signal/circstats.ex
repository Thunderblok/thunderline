defmodule Thunderline.Thunderbolt.Signal.CircStats do
  @moduledoc "Circular statistics: Rayleigh p-value + mean direction. (migrated from Thunderline.Current.CircStats)"
  @two_pi 2.0 * :math.pi()
  def rayleigh([]), do: {0.0, 0.0, 1.0}
  def rayleigh(phases) when is_list(phases) do
    n = length(phases)
    {sx, sy} = Enum.reduce(phases, {0.0, 0.0}, fn p, {cx, cy} ->
      a = p * @two_pi

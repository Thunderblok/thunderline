defmodule Thunderline.Thunderbolt.Resources.ChunkHealth do
  ...
  calculations do
    calculate :memory_usage_percent, :decimal, expr(
      memory_usage_mb / memory_limit_mb * 100
    )

    calculate :overall_health_score, :decimal, expr(
      (health_score +
  ...
  actions do
    ...
    create :record_health_check do
      accept [
        :status, :health_score, :cpu_usage_percent, :memory_usage_mb,
        :network_throughput_kbps, :response_time_ms, :throughput_ops_per_second,
        :error_rate_percent, :active_bit_count, :dormant_bit_count,
        :activation_failures, :signal_routing_errors, :warnings, :errors,
        :thundercore_pulse_health, :neighbor_connectivity
      ]
    ...
    read :recent_health_checks do
      ...
      filter expr(
        last_check > ago(^arg(:hours_ago), :hour)
      )
    end
  end

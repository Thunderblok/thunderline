defmodule Thunderline.Thunderbolt.Resources.RuleOracle do
  ...
  calculations do
    calculate :inference_rate, :float, expr(
    ...
    calculate :accuracy, :decimal, expr(
      fragment("CASE WHEN ? > 0 THEN (1.0 - ?)::decimal ELSE 0.0 END",
        total_inferences, error_rate)
    ) do
  ...
  actions do
    ...
    create :create do
      accept [:name, :implementation, :description, :input_size, :hidden_size,
              :batch_size, :temperature, :rule_parameters]
    end
    ...
    update :record_inference do
      accept [:total_inferences, :avg_latency_us, :error_rate, :last_inference_at, :performance_metrics]
    end

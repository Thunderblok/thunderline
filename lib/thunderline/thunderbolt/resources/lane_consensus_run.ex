defmodule Thunderline.Thunderbolt.Resources.ConsensusRun do
  ...
  calculations do
    calculate :energy_per_step, :float, expr(
      fragment("CASE WHEN ? > 0 THEN ?::float / ? ELSE 0.0 END",
        convergence_steps, final_energy, convergence_steps)
    ) do
      description "Average energy reduction per step"
    end

    calculate :efficiency_score, :float, expr(
      fragment("CASE WHEN ? > 0 AND ? = true THEN (? - ?)::float / ? ELSE 0.0 END",
        max_steps, success, max_steps, convergence_steps, max_steps)
    ) do
  ...
  actions do
    ...
    create :create do
      accept [:region_id, :trigger_reason, :matrix_size, :coupling_strength,
              :initial_temperature, :final_temperature, :max_steps, :lane_configuration_id]
    end

    update :complete do
      accept [:final_energy, :convergence_steps, :success, :execution_time_ms,
              :spin_configuration, :metadata]
    end

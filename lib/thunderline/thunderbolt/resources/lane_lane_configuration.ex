defmodule Thunderline.Thunderbolt.Resources.LaneConfiguration do
  ...
  calculations do
    calculate :current_load, :decimal, expr(
      fragment("COALESCE((SELECT COUNT(*) FROM consensus_runs WHERE lane_configuration_id = ? AND status = 'running'), 0)::decimal / ?",
  ...
  actions do
    ...
    create :create do
      accept [:name, :lane_type, :lane_family, :rule_parameters, :performance_target,
              :max_concurrent_tasks, :priority_weight, :adaptive_bounds, :metadata]
    end

    update :update do
      accept [:name, :rule_parameters, :performance_target, :max_concurrent_tasks,
              :priority_weight, :adaptive_bounds, :metadata]
    end

defmodule Thunderblock.Resources.ZoneContainer do
  ...
  actions do
    ...
    create :create do
      ...
      accept [:zone_name, :zone_type, :coordinates, :capacity_config, :zone_config,
              :supervision_strategy, :max_restarts, :max_seconds, :neighbor_zones,
              :phase_assignment, :tags, :metadata, :cluster_node_id]
      ...
      change after_action(fn _changeset, zone, _context ->
        ...
        Phoenix.PubSub.broadcast(
          Thunderline.PubSub,
          "thunderblock:zones",
          {:zone_created, %{zone_id: zone.id, zone_name: zone.zone_name, node_id: zone.cluster_node_id}}
        )
        {:ok, zone}
      end)
    end
    ...
    update :update do
      ...
      accept [:zone_name, :zone_type, :capacity_config, :zone_config, :supervision_strategy,
              :max_restarts, :max_seconds, :neighbor_zones, :phase_assignment, :tags, :metadata]
    end
    ...
    action :restart, :struct do
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.ZoneContainer, input.arguments.id) do
          nil -> {:error, "Zone container not found"}
          zone ->
            current_time = DateTime.utc_now()
            current_count = zone.restart_count || 0

            updated_zone = Ash.update!(zone, :_restart_internal, %{
              status: :active,
            ...
            Phoenix.PubSub.broadcast(
              Thunderline.PubSub,
              "thunderblock:zones",
              {:zone_restarted, %{zone_id: updated_zone.id, zone_name: updated_zone.zone_name, restart_count: updated_zone.restart_count}}
            )
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.ZoneContainer, input.arguments.id) do
          ...
          zone ->
            ...
            updated_zone = Ash.update!(zone, :_mark_degraded_internal, %{
              status: :degraded,
            ...
            Phoenix.PubSub.broadcast(
              Thunderline.PubSub,
              "thunderblock:zones",
              {:zone_degraded, %{zone_id: updated_zone.id, zone_name: updated_zone.zone_name, health_score: updated_zone.health_score}}
            )
    ...
    action :mark_failed, :struct do
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.ZoneContainer, input.arguments.id) do
          nil -> {:error, "Zone container not found"}
          zone ->
            updated_zone = Ash.update!(zone, :_mark_failed_internal, %{
              status: :failed,
            ...
            |> case do
              {:ok, updated_zone} ->
                ...
                Phoenix.PubSub.broadcast(
                  Thunderline.PubSub,
                  "thunderblock:health",
                  {:zone_health_checked, %{zone_id: updated_zone.id, health_score: updated_zone.health_score, status: updated_zone.status}}
                )
                {:ok, updated_zone}
  ...
  postgres do
    ...
    check_constraints do
      ...
      check_constraint :valid_phase, "phase_assignment >= 0 AND phase_assignment <= 11 OR phase_assignment IS NULL"
    end
  end

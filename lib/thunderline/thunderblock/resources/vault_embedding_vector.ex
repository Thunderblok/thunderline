defmodule Thunderline.Thunderblock.Resources.VaultEmbeddingVector do
  ...
  actions do
    ...
    create :create_embedding do
      ...
      accept [:vector, :vector_model, :dimension, :content_hash,
      ...
      change manage_relationship(:memory_record_id, :memory_record, type: :append_and_remove, on_no_match: :ignore)
      change manage_relationship(:memory_node_id, :memory_node, type: :append_and_remove, on_no_match: :ignore)
      change manage_relationship(:knowledge_node_id, :knowledge_node, type: :append_and_remove, on_no_match: :ignore)
      change manage_relationship(:experience_id, :experience, type: :append_and_remove, on_no_match: :ignore)
      ...
      change fn changeset, _context ->
        ...
        if vector && declared_dim && length(vector) != declared_dim do
          Ash.Changeset.add_error(changeset, field: :vector,
            message: "Vector length (#{length(vector)}) does not match declared dimension (#{declared_dim})")
        else
          changeset
        end
      end
  ...
  calculations do
    calculate :vector_norm, :float, expr(
      fragment("sqrt(array_sum(array_pow(?, 2)))", vector)
    ) do

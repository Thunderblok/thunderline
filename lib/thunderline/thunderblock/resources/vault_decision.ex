defmodule Thunderline.Thunderblock.Resources.VaultDecision do
  ...
  actions do
    ...
    create :make_decision do
      accept [:agent_id, :type, :context, :input_data, :options_considered, :selected_option,
              :reasoning, :confidence_score, :risk_assessment, :expected_outcome,
              :decision_time_ms, :tags, :parent_decision_id]
    end
    ...
    read :by_confidence_range do
      ...
      filter expr(confidence_score >= ^arg(:min_confidence) and confidence_score <= ^arg(:max_confidence))
    end
  ...
  validations do
    ...
    validate numericality(:confidence_score, greater_than_or_equal_to: 0, less_than_or_equal_to: 1)
    validate numericality(:risk_assessment, greater_than_or_equal_to: 0, less_than_or_equal_to: 1)
    validate numericality(:outcome_accuracy, greater_than_or_equal_to: 0, less_than_or_equal_to: 1) do
  ...
  defp calculate_outcome_accuracy(expected, actual) when is_map(expected) and is_map(actual) do
    ...
    if MapSet.size(common_keys) == 0 do
      ...
      matches = Enum.count(common_keys, fn key ->

defmodule Thunderblock.Resources.SupervisionTree do
  ...
  actions do
    ...
    create :create do
      ...
      accept [:tree_name, :tree_type, :supervision_strategy, :max_restarts, :max_seconds,
              :child_specs, :recovery_strategy, :monitoring_config, :parent_tree_id,
              :depth_level, :tags, :metadata, :cluster_node_id, :zone_container_id]
      ...
      change after_action(fn _changeset, tree, _context ->
        ...
        Phoenix.PubSub.broadcast(
          ...
          {:tree_created, %{tree_id: tree.id, tree_name: tree.tree_name, zone_id: tree.zone_container_id}}
        )
        {:ok, tree}
      end)
    end
    ...
    update :update do
      ...
      accept [:tree_name, :supervision_strategy, :max_restarts, :max_seconds,
              :child_specs, :recovery_strategy, :monitoring_config, :tags, :metadata]
    end
    ...
    action :start, :struct do
      ...
      run fn input, _context ->
        case Thunderblock.Domain |> Ash.get(Thunderblock.Resources.SupervisionTree, input.arguments.id) do
          {:ok, tree} ->
            {:ok, updated_tree} = tree
    ...
    action :stop, :struct do
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.SupervisionTree, input.arguments.id) do
          nil -> {:error, "Supervision tree not found"}
          tree ->
            updated_tree = Ash.update!(tree, :_stop_internal, %{
              status: :stopped,
              running_children: []
            })
        ...
        case Ash.update!(changeset) do
          tree ->
            ...
            Phoenix.PubSub.broadcast(
              Thunderline.PubSub,
              "thunderblock:supervision",
              {:tree_restarted, %{tree_id: tree.id, restart_count: tree.restart_count, escalation_level: tree.escalation_level}}
            )
            {:ok, tree}
        end
      end
    ...
    action :handle_child_failure do
      ...
      run fn changeset, context ->
        ...
        updated_failed = [new_failure | current_failed] |> Enum.take(100)  # Keep last 100 failures
        ...
        case Ash.update!(changeset) do
          tree ->
            Phoenix.PubSub.broadcast(
              Thunderline.PubSub,
              "thunderblock:supervision",
              {:child_failed, %{tree_id: tree.id, child_id: child_id, reason: failure_reason, health_score: tree.health_score}}
            )
            {:ok, tree}
        end
      end
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.SupervisionTree, input.arguments.id) do
          ...
          tree ->
            ...
            updated_tree = Ash.update!(tree, :_add_child_internal, %{
      ...
      run fn input, _context ->
        case Ash.get!(Thunderblock.Resources.SupervisionTree, input.arguments.id) do
          ...
          tree ->
            ...
            updated_tree = Ash.update!(tree, :_remove_child_internal, %{
              running_children: updated_running
            })
  ...
  postgres do
    ...
    custom_indexes do
      index [:tree_name, :zone_container_id], unique: true, name: "supervision_trees_name_zone_idx"
    ...
    check_constraints do
      ...
      check_constraint :valid_restart_counts, "restart_count >= 0 AND max_restarts >= 0 AND max_seconds > 0"
  ...
  defp escalate_if_needed(changeset) do
    ...
    status = if new_escalation >= 5, do: :failing, else: Ash.Changeset.get_attribute(changeset, :status)
  ...
  defp calculate_health_score_from_failures(changeset) do
    ...
    recent_failures = Enum.count(failed_children, fn failure ->

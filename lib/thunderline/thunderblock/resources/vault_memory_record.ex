defmodule Thunderline.Thunderblock.Resources.VaultMemoryRecord do
  ...
  actions do
    ...
    create :store_memory do
      ...
      accept [:summary, :full_content, :memory_type, :context_data, :sensory_data,
              :emotional_data, :vividness, :accessibility, :importance, :memory_tags]
    ...
    update :fade_memory do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.change_attribute(:accessibility, max(0.1, current_accessibility * fade_factor))
      end
  ...
  calculations do
    ...
    calculate :age_in_days, :integer, expr(fragment("EXTRACT(EPOCH FROM (NOW() - inserted_at)) / 86400")) do

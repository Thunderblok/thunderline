defmodule Thunderline.Thunderblock.MigrationRunner do
  ...
  defp run_migrations(_path, 0, _base_backoff, attempt), do: raise "[MigrationRunner] Exhausted migration attempts at attempt #{attempt}"
  ...
  defp parse_repo_conn_info(repo_cfg) do
    ...
    if is_binary(url) do
      ...
      %{db: db, host: uri.host || Keyword.get(repo_cfg, :hostname, "127.0.0.1"), port: uri.port || Keyword.get(repo_cfg, :port, 5432), user: user || Keyword.get(repo_cfg, :username, System.get_env("PGUSER", "postgres")), pass: pass || Keyword.get(repo_cfg, :password, System.get_env("PGPASSWORD", "postgres"))}
    else
      %{db: Keyword.get(repo_cfg, :database), host: Keyword.get(repo_cfg, :hostname, "127.0.0.1"), port: Keyword.get(repo_cfg, :port, 5432), user: Keyword.get(repo_cfg, :username, System.get_env("PGUSER", "postgres")), pass: Keyword.get(repo_cfg, :password, System.get_env("PGPASSWORD", "postgres"))}
    end
  end
end

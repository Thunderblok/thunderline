defmodule Thunderline.Thundergate.Resources.ThunderboltMonitor do
  ...
  attributes do
    ...
    attribute :service_type, :atom, constraints: [one_of: [:chunk_manager, :load_balancer, :orchestrator, :coordinator]]
  ...
  actions do
    ...
    update :heartbeat do
      accept [:load_percentage, :active_connections, :pending_tasks, :completed_tasks,
              :failed_tasks, :average_response_time_ms, :throughput_per_minute,
              :memory_usage_mb, :cpu_usage_percent, :disk_usage_percent,
              :network_bandwidth_mbps, :chunk_count, :chunk_size_mb, :health_score]
    ...
    action :capacity_alert do
      ...
      run fn input, _context ->
        ...
        alerts = if service.cpu_usage_percent > cpu_threshold do
          [%{type: :cpu, value: service.cpu_usage_percent, threshold: cpu_threshold} | alerts]
        else
          alerts
        end

        alerts = if service.memory_usage_mb && service.memory_usage_mb > memory_threshold do
          [%{type: :memory, value: service.memory_usage_mb, threshold: memory_threshold} | alerts]
        else
          alerts
        end

        alerts = if service.load_percentage > load_threshold do
    ...
    read :stale_heartbeats do
      filter expr(last_heartbeat < ago(120, :second)) # 2 minutes
    end
  ...
  calculations do
    ...
    calculate :uptime_hours, :float, expr(
      fragment("EXTRACT(EPOCH FROM (? - ?))/3600", last_updated, registered_at)
    )

    calculate :task_success_rate, :float, expr(
      fragment("CASE WHEN (? + ?) > 0 THEN (?::float / (? + ?)::float) * 100 ELSE 100 END",
        completed_tasks, failed_tasks, completed_tasks, completed_tasks, failed_tasks)
    )

    calculate :error_rate, :float, expr(
      fragment("CASE WHEN ? > 0 THEN (?::float / ?::float) * 100 ELSE 0 END",
        completed_tasks, error_count, completed_tasks)
    )

    calculate :capacity_utilization, :float, expr(
      fragment("(COALESCE(?, 0) + COALESCE(?, 0) + COALESCE(?, 0)) / 3",
        load_percentage, cpu_usage_percent, memory_usage_mb)
    )
    ...
    calculate :heartbeat_age_seconds, :integer, expr(
      fragment("EXTRACT(EPOCH FROM (? - ?))", now(), last_heartbeat)
    )
  end

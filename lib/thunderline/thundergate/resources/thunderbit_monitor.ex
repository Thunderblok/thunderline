defmodule Thunderline.Thundergate.Resources.ThunderbitMonitor do
  ...
  actions do
    ...
    read :stale_agents do
      # 5 minutes
      filter expr(last_tick < ago(300, :second))
    end
  end
  ...
  attributes do
    ...
    attribute :status, :atom,
      constraints: [one_of: [:active, :idle, :processing, :error, :terminated]]
  ...
  calculations do
    calculate :is_active,
              :boolean,
              expr(status in [:active, :processing] and last_tick > ago(60, :second))

    calculate :uptime_minutes,
              :integer,
              expr(fragment("EXTRACT(EPOCH FROM (? - ?))/60", last_updated, monitored_since))

    calculate :ticks_per_minute,
              :float,
              expr(
                fragment(
                  "? / GREATEST(EXTRACT(EPOCH FROM (? - ?))/60, 1)",
                  tick_count,
                  last_updated,
                  monitored_since
                )
    ...
    calculate :distance_from_origin,
              :float,
              expr(
                fragment(
                  "SQRT(POWER(COALESCE(?, 0), 2) + POWER(COALESCE(?, 0), 2) + POWER(COALESCE(?, 0), 2))",
                  position_x,
                  position_y,
                  position_z
                )

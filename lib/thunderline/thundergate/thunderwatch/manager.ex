defmodule Thundergate.Thunderwatch.Manager do
  ...
  def init(_opts) do
    ...
    Logger.info(
      "[Thunderwatch] started roots=#{inspect(roots)} hash?=#{hash?} (Thundergate domain)"
    )
  ...
  defp start_fs_watcher(root) do
    try do
      ...
      e ->
        Logger.error(
          "[Thunderwatch] failed watcher start root=#{root} error=#{Exception.message(e)}"
        )
  ...
  defp via_name(root),
    do:
      String.to_atom(
        "thunderwatch_" <> Base.encode16(:erlang.phash2(root) |> :erlang.term_to_binary())
      )

  defp scan_dir(dir, ignore, hash?) do
    pattern = Path.join([dir, "**", "*"])

    for path <- Path.wildcard(pattern, match_dot: true),
        File.regular?(path),
        not ignored?(path, ignore) do
  ...
  defp file_meta(path, hash?) do
    case File.stat(path) do
      {:ok, %File.Stat{mtime: mtime, size: size, type: type}} ->
        %{mtime: mtime, size: size, type: type, hash: if(hash?, do: safe_hash(path), else: nil)}
  ...
  defp infer_domain(path),
    do:
      Enum.find_value(@domain_markers, :system, fn {dom, marker} ->
        if String.contains?(path, marker), do: dom
      end)

defmodule Thunderline.ObanDiagnostics do
  ...
  Provides verbose logging every @interval ms about:
  - Repo status & database connectivity
  - Presence (or absence) of the Oban supervisor process
  - Applied Oban migrations (existence checks on oban_jobs / oban_peers)
  - Optional insertion of a demo job to validate writes

  Disable by setting OBAN_DIAGNOSTICS=0.
  """
  use GenServer
  require Logger
  ...
  defp check_tables do
    for table <- ["oban_jobs", "oban_peers"] do
      case Thunderline.Repo.query("SELECT to_regclass($1)", [table]) do
        # When present, Postgres returns a regclass OID (integer) or the text of the name depending on driver mapping.
        {:ok, %{rows: [[nil]]}} ->
          Logger.error("[ObanDiagnostics] MISSING table #{table} – migration not applied")
        {:ok, %{rows: [[present]]}} when not is_nil(present) ->
          if verbose?(), do: Logger.debug("[ObanDiagnostics] table_present=#{table} regclass=#{inspect(present)}")
        {:error, err} ->
          Logger.error("[ObanDiagnostics] table_check_error=#{table} error=#{inspect(err)}")
      end
    end
  end

  defp log_queue_overview do
    # Minimal queue stats: count available jobs per configured queue
    queues = configured_queues()
    Enum.each(queues, fn {queue, _concurrency} ->
      case Thunderline.Repo.query("SELECT count(*) FROM oban_jobs WHERE queue=$1 AND state='available'", [to_string(queue)]) do
        {:ok, %{rows: [[count]]}} ->
          if verbose?(), do: Logger.debug("[ObanDiagnostics] queue=#{queue} available=#{count}")
        {:error, err} ->
          Logger.warning("[ObanDiagnostics] queue_stat_error queue=#{queue} error=#{inspect(err)}")
      end
    end)
  end

  defp configured_queues do
    Application.get_env(:thunderline, Oban, []) |> Keyword.get(:queues, [])
  end
  ...
  defp attempt_demo_job_insert do
    ...
    case Code.ensure_loaded?(Thunderline.Workers.DemoJob) do
      ...
      false ->
        if verbose?(), do: Logger.debug("[ObanDiagnostics] DemoJob module not loaded; skipping test insert")
    end
  end

defp verbose? do
  System.get_env("OBAN_DIAGNOSTICS_VERBOSE") in ["1", "true", "TRUE", "yes", "Y"]
end
end

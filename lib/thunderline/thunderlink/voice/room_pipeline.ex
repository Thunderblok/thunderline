defmodule Thunderline.Thunderlink.Voice.RoomPipeline do
  ...
  @impl true
  def handle_cast({:offer, principal_id, sdp}, state), do: (broadcast(state.room_id, {:webrtc_offer, principal_id, sdp}); {:noreply, state})
  def handle_cast({:answer, principal_id, sdp}, state), do: (broadcast(state.room_id, {:webrtc_answer, principal_id, sdp}); {:noreply, state})
  def handle_cast({:ice, principal_id, cand}, state), do: (broadcast(state.room_id, {:webrtc_candidate, principal_id, cand}); {:noreply, state})
  def handle_cast({:speaking, principal_id, speaking?}, state), do: (broadcast(state.room_id, {:speaking, principal_id, speaking?}); {:noreply, state})
  defp broadcast(room_id, event), do: Phoenix.PubSub.broadcast(Thunderline.PubSub, "voice:#{room_id}", event)
end

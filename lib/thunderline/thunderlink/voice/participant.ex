defmodule Thunderline.Thunderlink.Voice.Participant do
  ...
  defp ensure_role(changeset, _ctx) do
    ...
    if role in [:host, :speaker, :listener],
      do: changeset,
      else: Ash.Changeset.change_attribute(changeset, :role, :listener)
  end
  ...
  defp validate_promotion(changeset, _ctx) do
    ...
    if role in [:host, :speaker, :listener],
      do: changeset,
      else: Ash.Changeset.add_error(changeset, field: :role, message: "invalid role")
  end

  defp touch(changeset, _ctx),
    do: Ash.Changeset.change_attribute(changeset, :last_active_at, DateTime.utc_now())

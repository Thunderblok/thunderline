defmodule Thunderline.Thunderlink.Presence.Enforcer do
  ...
  `Thunderline.Thundergate.Policies.Presence.decide/3`.
  ...
  defmacro with_presence(action, resource, actor_ctx) do
    quote do
      case Presence.decide(unquote(action), unquote(resource), unquote(actor_ctx)) do
        {:allow, meta} ->
          {:ok, meta}
        ...
        {:deny, reason} ->
          :telemetry.execute(
            ...
            %{count: 1},
            %{action: unquote(action), resource: unquote(resource), reason: reason}
          )

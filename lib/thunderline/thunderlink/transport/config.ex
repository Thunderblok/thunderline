defmodule Thunderline.Thunderlink.Transport.Config do
  @moduledoc """
  Transport (TOCP) configuration accessor under the Thunderlink namespace.

  Normalizes the existing keyword list in `Application.get_env(:thunderline, :tocp)`
  into a structured map and caches it in `:persistent_term` for fast reads.
  """
  ...
  @doc "Get a nested config value by path, e.g., [:reliable, :window]."

defmodule Thunderline.Thunderlink.Transport.Routing.SwitchTracker do
  ...
  def handle_cast({:switch, zone, prev, new, total_nodes, _opts}, state) do
    cond do
      prev == nil or new == nil or prev == new ->
        {:noreply, state}

      true ->
        zones =
          Map.update(state.zones, zone, %{switches: 1, total_nodes: total_nodes}, fn z ->
  ...
  def handle_info(:flush, state) do
    ...
    Enum.each(state.zones, fn {zone, %{switches: sw, total_nodes: tn}} ->
      ...
      :telemetry.execute([:tocp, :routing_relay_switch_rate], %{rate_pct: rate_pct}, %{
        zone: zone,
        switches: sw,
        total_nodes: tn || 0,
        window_s: window_s

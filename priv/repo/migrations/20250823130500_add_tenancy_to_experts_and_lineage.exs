defmodule Thunderline.Repo.Migrations.AddTenancyToExpertsAndLineage do
  ...
  defp try_db_backfill do
    ...
    has_uuid_ossp = repo.query!("SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname='uuid-ossp')").rows |> hd() |> hd()
    has_pgcrypto = repo.query!("SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname='pgcrypto')").rows |> hd() |> hd()

    cond do
      has_uuid_ossp ->
        repo.query!("UPDATE experts       SET tenant_id = uuid_generate_v4() WHERE tenant_id IS NULL")
        repo.query!("UPDATE lineage_edges SET tenant_id = uuid_generate_v4() WHERE tenant_id IS NULL")
      has_pgcrypto ->
        repo.query!("UPDATE experts       SET tenant_id = gen_random_uuid() WHERE tenant_id IS NULL")
        repo.query!("UPDATE lineage_edges SET tenant_id = gen_random_uuid() WHERE tenant_id IS NULL")
      true ->

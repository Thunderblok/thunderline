...
def load_keras_model(model_path: str) -> Tuple[Any, Dict[str, Any]]:
    ...
    Handles both:
    - CerebrosNotGPT wrapper models (extracts inner model)
    - Plain Keras models
    ...
    is_cerebros_wrapper = model_type == "CerebrosNotGPT" or hasattr(loaded_model, 'config')
    
    if is_cerebros_wrapper:
        print(f"   ✓ Detected CerebrosNotGPT wrapper model")
        # Extract the inner inference model
        if hasattr(loaded_model, 'model') and loaded_model.model is not None:
            model = loaded_model.model
            print(f"   ✓ Extracted inner model for ONNX export")
        else:
            raise ValueError("CerebrosNotGPT wrapper has no inner model - cannot export")
        
        # Extract config from wrapper
        cerebros_config = {
            "max_sequence_length": getattr(loaded_model, 'max_sequence_length', None),
            "padding_token": getattr(loaded_model, 'padding_token', None),
        }
    else:
        print(f"   ✓ Plain Keras model detected")
        model = loaded_model
        cerebros_config = None
    
    # Extract configuration from the actual inference model
    config = {
        "input_shape": None,
        "output_shape": None,
        "num_parameters": model.count_params(),
        "is_cerebros_wrapper": is_cerebros_wrapper,
        "cerebros_config": cerebros_config,
    }
...
def generate_metadata(
    ...
    metadata = {
        ...
        "is_cerebros_wrapper": config.get("is_cerebros_wrapper", False),
        "cerebros_config": config.get("cerebros_config"),
        "inputs": [],
        "outputs": [],
        "thunderline_compatible": True,
        "notes": [
            "This ONNX model outputs raw logits/probabilities for next token prediction.",
            "The model predicts ONE token at a time (single-head architecture).",
            "For text generation, implement an autoregressive loop with sampling.",
            "Input: token IDs padded to max_seq_length",
            "Output: probabilities over vocabulary (shape: [batch, vocab_size])"
        ],
        "usage": {
            ...
            "python_generation": 'See onnx_inference.py for full text generation with sampling',
        }
    }
    ...
    for out in onnx_model.graph.output:
        ...
        metadata["outputs"].append({
            ...
            "shape": shape,
            "dtype": "float32",
            "description": "Next-token probabilities (softmax output) over vocabulary"
        })
